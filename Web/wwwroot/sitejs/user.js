...
function UpdateProfile(_this) {
    ...
    let frmValid;
    if ($("#UserId").val() > 0) {
        if ($('#imgUser').attr('src') != defaultImageUrl) {
            IsImageSelected = true;
        }
        else {
            $("#images").parent().next(".text-danger").remove();
            $("#images").parent().after('<span class="arrMessage text-danger field-validation-error" data-valmsg-for="ProfileImage" data-valmsg-replace="true">Profile Image is required.</span>');
            IsImageSelected = false;
        }
    }
    let img = new Image();
    img.src = $('#imgUser').attr('src');
    ...
    if ($("#frmUpdateprofile").valid() && frmValid) {
